...
class PRPDWindow(QMainWindow):
        ...
        try:
            if ph.size and amp.size:
                if color_points and n_clusters > 0:
                    ...
                    import numpy as _np
                    # Construir robustamente el arreglo de centros evitando errores de conversión
                    centers_list = []
                    for c in clouds:
                        # Convertir coordenadas a float, con fallback a 0.0 si falla
                        try:
                    ...
                    centers = _np.array(centers_list, dtype=float)
                    # Inicializar etiquetas para cada punto (tamaño igual que ph)
                    ...
                    try:
                        ...
                        for j in range(n_clusters):
                            m = (lbl == j)
                            # Usar Python any() en lugar de np.any() para evitar problemas si m no es un array
                            if bool(m.any() if hasattr(m, 'any') else any(m)):
                                color = palette[j % len(palette)]
                                label = f"C{j + 1}" if j < max_labels else None
                                ax.scatter(ph[m], amp[m], s=6, alpha=0.65, color=color, label=label)
                    except Exception:
                        # Fallback: trazar todos los puntos en gris claro en caso de error
    ...
    def render_result(self, r: dict) -> None:
        ...
        if view_mode.startswith("histogramas"):
            ...
            base_for_s5 = clouds_s4 if clouds_s4 else clouds_s3
            clouds_s5 = select_dominant_clouds(base_for_s5) if base_for_s5 else []
    ...
    def _export_session_outputs(self, result: dict, session_dir: Path, profile_tag: str) -> None:
        ...
        s3_eps = float(result.get("s3_eps", 0.045))
        s3_min_samples = int(result.get("s3_min_samples", 10))
        s3_force_multi = bool(result.get("s3_force_multi", False))
        clouds_s3 = pixel_cluster_clouds(ph_al, amp_al, eps=s3_eps, min_samples=s3_min_samples, force_multi=s3_force_multi) if (ph_al.size and amp_al.size) else []
        clouds_s4 = combine_clouds(clouds_s3) if clouds_s3 else []
        base_for_s5 = clouds_s4 if clouds_s4 else clouds_s3
        clouds_s5 = select_dominant_clouds(base_for_s5) if base_for_s5 else []
