...
class PRPDWindow(QMainWindow):
    ...
    def run_pipeline(self) -> None:
        ...
        try:
            ...
            # Procesar PRPD utilizando el offset de fase y filtro
    ...
    def render_result(self, r: dict) -> None:
        ...
        if view_mode.startswith("angpd"):
            # En vista ANGPD se dibujan los histogramas de amplitud y fase, y las curvas ANGPD ponderadas por quantity.
            try:
            ...
            # Graficar ANGPD/N-ANGPD ponderado por quantity en el panel inferior izquierdo
            ang = r.get("angpd", {})
            x = np.asarray(ang.get("phi_centers", []), dtype=float)
            y_q_ang = np.asarray(ang.get("angpd_qty", []), dtype=float)
            ...
            if x.size and (y_q_ang.size or y_q_n.size):
                ...
                if y_q_ang.size:
                    # Escalar por 100 para mejorar la visibilidad, igual que en la referencia
                    self.ax_probs.plot(x, y_q_ang * 100.0, label="ANGPD qty (sum=1) x100", color="#2ca02c")
                self.ax_probs.set_xlim(0, 360)
                self.ax_probs.set_xlabel("Fase (°)")
                # Eje y único para ambas curvas (densidad y densidad x100). Etiquetar según convenga
                self.ax_probs.set_ylabel("Densidad / Densidad(x100)")
                ...
                # Si no hay quantity en el XML, avisar al usuario en este panel
